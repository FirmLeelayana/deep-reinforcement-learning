...
def main_quantitative():
    ...
    # No. batches until convergence = 4000
    ...
    mean_vector, variance_vector = evaluation.create_evaluation_table_overall(number_batches=4000, episodes_per_batch=100, 
...
def main_qualitative():
    ...
    evaluation.qualitative_evaluation_plots(number_batches=4000, episodes_per_batch=100, test_type='overall')

    # Produce plots on a single test epsiode, which contains samples of only the trained-on/seen failure modes
    evaluation.qualitative_evaluation_plots(number_batches=4000, episodes_per_batch=100, test_type='seen')

    # Produce plots on a single test epsiode, which contains samples of only the unseen failure modes.
    evaluation.qualitative_evaluation_plots(number_batches=4000, episodes_per_batch=100, test_type='unseen')
