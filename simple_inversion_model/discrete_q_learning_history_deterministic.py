# This file implements the 7th solution, a simple tabular q learning algorithm to the simple inversion state space model, 
# with both discrete action and state spaces, where only a single mode of failure exists.
# Deterministic update, history buffer version.

import numpy as np
import random
import matplotlib.pyplot as plt

class DiscreteQLearningHistoryBufferDeterministic:
    """
    ...
    def run_one_episode_and_train(self):
        ...
        for k in range(2, self.time_steps):  # going through the time steps from k=2 onwards (as we need to initialize at k=0)
            if random.uniform(0, 1) < self.epsilon:  # exploring with probability epsilon (epsilon greedy)
                ...
                min_cost_index = np.argmin(self.q_table[int(self.x[k] + self.x_limit), int(self.x[k-1] + self.x_limit), int(self.x[k-2] + self.x_limit), 
                                                        int(self.u[k-1] + self.u_limit), int(self.u[k-2] + self.u_limit)])
        ...
        for k in range(2, self.time_steps):
            ...
            self.number_times_explored[int(self.x[k] + self.x_limit), int(self.x[k-1] + self.x_limit), int(self.x[k-2] + self.x_limit), 
                                       int(self.u[k-1] + self.u_limit), int(self.u[k-2] + self.u_limit), int(self.u[k] + self.u_limit)] += 1
            ...
            for a in a_vector:
                ...
                for k in range(2, self.time_steps):
                    ...
                    min_cost_index = np.argmin(self.q_table[int(x_values[k] + self.x_limit), int(x_values[k-1] + self.x_limit), int(x_values[k-2] + self.x_limit), 
                                                            int(u_values[k-1] + self.u_limit), int(u_values[k-2] + self.u_limit)])
