...
# evaluation metrics for the discrete_q_learning_action_noise case, as specified in the 'Evaluating RL algorithms' document.
...
from discrete_q_learning_action_noise import DiscreteQLearningActionNoise  # import RL agent class
...
def main_quantitative():
    ...
    # No. batches until convergence = 5000

    agent = DiscreteQLearningActionNoise()  # create agent
    ...
    mean_vector, variance_vector = evaluation.create_evaluation_table_overall(number_batches=5000, episodes_per_batch=100, 
...
def main_qualitative():
    ...
    evaluation.qualitative_evaluation_plots(number_batches=5000, episodes_per_batch=100, test_type='overall')

    # Produce plots on a single test epsiode, which contains samples of only the trained-on/seen failure modes
    evaluation.qualitative_evaluation_plots(number_batches=5000, episodes_per_batch=100, test_type='seen')

    # Produce plots on a single test epsiode, which contains samples of only the unseen failure modes.
    evaluation.qualitative_evaluation_plots(number_batches=5000, episodes_per_batch=100, test_type='unseen')
