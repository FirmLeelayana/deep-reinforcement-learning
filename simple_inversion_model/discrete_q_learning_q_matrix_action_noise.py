# This file implements the 8th solution, a simple tabular q learning algorithm to the simple inversion state space model, 
# with both discrete action and state spaces, with discrete additive noise on the action values to encourage exploration
# of untouched states in the q matrix, as well as additive action noise on the q matrix index.

import numpy as np
import random
import matplotlib.pyplot as plt

class DiscreteQLearningActionNoiseQMatrix:
    """
    ...
    def run_one_episode_and_train(self):
        ...
        for k in range(1, self.time_steps):  # going through the time steps from k=2 onwards (as we need to initialize at k=0)
            ...
            self.u[k] += np.random.choice(self.action_noise, p=self.probability_noise)  # add random additive integer noise to the action
...
if __name__ == "__main__":
    ...
    agent = DiscreteQLearningActionNoiseQMatrix(number_of_episodes_per_batch=100, number_of_batches=10000)  # (1) X = number of batches until convergence
