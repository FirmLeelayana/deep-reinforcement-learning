# Combining both DQN and Q-learning approach.
...
class DQNQL:
    ...
    def __init__(self, x_limit=10, u_limit = 3, time_steps=10, epsilon=1, 
                 ...
                 self.number_of_episodes_per_batch_q_learning = 100000

                 # Initialize cost per batch vector
                 self.cost_per_batch = []

                 # Q matrix
                 self.q_table = np.zeros((2*x_limit + 1, 2*x_limit + 1, 2*u_limit + 1, 2*u_limit + 1))
    ...
    def run_one_episode_and_train(self):
        ...
        for k in range(1, self.time_steps):  # going through the time steps from k=2 onwards (as we need to initialize at k=0)
            if random.uniform(0, 1) < self.epsilon_q:  # exploring with probability epsilon (epsilon greedy)
    ...
    def run_one_batch_and_train(self):
        ...
        for i in range(self.number_of_episodes_per_batch_q_learning):
        ...
        for b in self.possible_b_vector:
            for a in a_vector:
                ...
                for k in range(1, self.time_steps):
                    ...
                    # DQN action
                    dqn_action = max_q_index - self.u_limit
                    ...
                    min_cost_index_value = np.min(self.q_table[int(x_values[k] + self.x_limit), int(x_values[k-1] + self.x_limit), int(u_values[k-1] + self.u_limit)])
                    q_matrix_action = min_cost_index - self.u_limit

                    # Normalizing state-action magnitudes
                    max_q_value_norm = max_q_value / (max_q_value + min_cost_index_value)
                    min_cost_index_value_norm = min_cost_index_value / (max_q_value + min_cost_index_value)

                    # Calculating weights
                    dqn_weight = max_q_value_norm / (max_q_value_norm + min_cost_index_value_norm)

                    # Weighted combination of the DQN action and the Q-matrix action
                    u_values[k] = np.clip(int(dqn_weight * dqn_action + (1-dqn_weight) * q_matrix_action), -self.u_limit, self.u_limit)
            ...
            for b in self.possible_b_vector:
                for a in a_vector:
                    ...
                    for k in range(1, self.time_steps):
                        ...
                        # DQN action
                        max_q_index = np.argmax(current_q_values)
                        max_q_index_value = np.max(current_q_values)
                        dqn_action = max_q_index - self.u_limit
                        ...
                        # Initialize weight - how much we value the q-learning values over the DQN algorithm. The higher, the more we value.
                        weight = 10000

                        # Initialize factor
                        factor = (min_cost_index_value * weight) / (min_cost_index_value * weight + max_q_index_value)

                        # Weighted combination of the DQN action and the Q-matrix action -> will try to always pick q_matrix_action, except when it has never explored that index,
                        # which is when min_cost_index_value will be 0, therefore it will then pick the dqn action. Here we are trying to make it such that the algorithm goes with
                        # q-learning most of the time, except in areas it has not explored the q-matrix, thus it will then default to the DQN with its better generalization properties.
                        u_values[k] = np.clip(int((1-factor) * dqn_action + factor * q_matrix_action), -self.u_limit, self.u_limit)

                        # Basically limits x to x_limit and -x_limit for next state, and updates next state
                        x_values[k+1] = np.clip(a * x_values[k] + b * u_values[k], -self.x_limit, self.x_limit)
...
if __name__ == "__main__":
    ...
    agent = DQNQL(number_of_episodes_per_batch=10, number_of_batches=100000)  # (1) X = number of batches until convergence
