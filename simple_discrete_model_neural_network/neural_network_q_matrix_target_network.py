...
class DQN:
    ...
    def run_one_episode_and_train(self):
        ...
        with tf.device('/device:GPU:0'):
            ...
            for k in range(1, self.time_steps):
                ...
                next_q_values = self.target_model.predict(next_state)[0]    # grab the q-values over all possible actions in the next state, outputted by NN.
    ...
    def run_one_batch_and_train(self):
        ...
        # Update target model with main model after every batch
        self.target_model.set_weights(self.model.get_weights())
...
if __name__ == "__main__":
    ...
    # Target model is updated with main model's weights after every batch of training -> help with making the samples more stationary.
