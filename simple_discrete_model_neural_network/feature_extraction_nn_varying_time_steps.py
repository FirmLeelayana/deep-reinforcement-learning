...
class DQN:
    ...
    def __init__(self, x_limit=10, u_limit = 3, time_steps=11, epsilon=1, 
                 ...
                 self.state_size = 5    # Augmented state = xk xk-1 uk-1, 3 parameters
    ...
    def run_one_episode_and_append_to_memory(self):
        ...
        with tf.device('/device:GPU:0'):
            ...
            for k in range(1, self.time_steps):  # going through the time steps from k=1 onwards (as we need to initialize at k=0)
                # Get current state
                current_state = np.array([self.x[k], self.x[k-1], self.u[k-1], self.x[k-2], self.u[k-2]])
                ...
                next_state = np.array([self.x[k+1], self.x[k], self.u[k], self.x[k-1], self.u[k-1]])
