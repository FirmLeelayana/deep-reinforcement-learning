# Teacher student network training proof
...
class DQN:
    ...
    def __init__(self, x_limit=10, u_limit = 10, time_steps=10, epsilon=1, 
                 possible_b_vector=[1,-1], possible_a_vector=[2,-2], 
                 number_of_episodes_per_batch=100, number_of_batches=5000,
                 unseen_a_vector=[1, -1], q_table = None):
                 ...
                 # Use learnt q-matrix from q-learning
    ...
    def run_one_episode_and_append_to_memory(self):
        ...
        with tf.device('/device:GPU:0'):
            ...
            for k in range(1, self.time_steps):  # going through the time steps from k=1 onwards (as we need to initialize at k=0)
                ...
                # Choose minimum cost action, minimised over all the possible actions (u(k))
                selected_action = np.argmin(self.q_table[int(self.x[k] + self.x_limit), int(self.x[k-1] + self.x_limit), int(self.u[k-1] + self.u_limit)])
...
if __name__ == "__main__":
    ...
    # Train q-learning solution to optimal, at 37000 batches required
    q_learning_agent.run_multiple_batches_and_plot(batch_number_until_plot=10, option = 'trajectory')
    plt.pause(5)  # Pause the final plot for 5 seconds

    # Now transfer this knowledge over to DQN
    dqn_agent = DQN(number_of_episodes_per_batch=10, number_of_batches=100000, q_table = q_learning_agent.q_table)
    dqn_agent.run_multiple_batches_and_plot(batch_number_until_plot=1, option = 'trajectory')
